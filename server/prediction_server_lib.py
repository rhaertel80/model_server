...
def _get_model_size(model_dir):
  ...
  # TODO(b/37792601): Get model size from env var or version proto.
...
class _InferenceHandler(webapp2.RequestHandler):
  ...
  def _instrumented_post(self, stats):
    ...
    with stats.time(LOADS_TIME):
      try:
        ...
        if not self.request.body.strip():
          # TODO(b/36457049): Consider to return a response in API server for an
          # empty request.
    ...
    with stats.time(DECODE_TIME):
      try:
        # TODO(b/67753059): Investigate whether decoding should happen for all
        # models, or it should be a TF-specific preprocessing.
